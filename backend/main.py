...
async def evaluate(
    ...
    try:
        ...
        pdf.set_xy(10, 10)
        pdf.cell(190, 10, txt=str(t["title"]), ln=True, align="C")
        pdf.ln(10)

        pdf.set_font(font_name, "", 12)
        text = (
            f"{t['horizontal']}: {horPercent:.1f}% ({left:.2f} mm / {right:.2f} mm)\n"
            f"{t['vertical']}: {verPercent:.1f}% ({top:.2f} mm / {bottom:.2f} mm)\n"
            f"{t['global']}: {globalPercent:.1f}%\n\n"
            f"{t['psa']}: {str(psa)}\n"
            f"{t['bgs']}: {str(bgs)}\n"
            f"{t['sgc']}: {str(sgc)}"
        )
